...
ScriptEngineNode::ScriptEngineNode(const QString &name,
                                   const QString &description,
                                   ScriptEngineNode::NodeType type,
...
ScriptEngineNode *ScriptEngineNode::addChild(const QString &name,
                                             const QString &description,
                                             ScriptEngineNode::NodeType type,
                                             const QIcon &icon) {
...
void ScriptEngineModel::updateModel() {
  ...
  foreach(ScriptFunctionDesc funcDesc, m_scriptManager->functionsDescribers()) {
    ...
    if(categories.contains(categoryName)) {
      ...
      categ = m_rootNode->addChild(categoryName,
                                   "",
                                   ScriptEngineNode::Category,
                                   QIcon(":/report/images/folder"));
      categories.insert(categoryName, categ);
    }
    categ->addChild(funcDesc.name,
                    funcDesc.description,
                    ScriptEngineNode::Function,
...
bool ScriptEngineManager::addFunction(const JSFunctionDesc &functionDescriber) {
  if(m_functions.contains(functionDescriber.name()))
    return false;
  ScriptValueType functionManager = scriptEngine()->globalObject().property(
  ...
  if(!functionManager.isValid()) {
    ...
    scriptEngine()->globalObject().setProperty(functionDescriber.managerName(), functionManager);
  }
...
QString ScriptEngineManager::expandUserVariables(QString context,
                                                 RenderPass /* pass */,
...
QString ScriptEngineManager::expandDataFields(QString context,
                                              ExpandType expandType,
                                              QVariant &varValue,
                                              QObject *reportItem) {
#if(QT_VERSION < QT_VERSION_CHECK(5, 15, 1))
...
QString ScriptEngineManager::expandScripts(QString context,
                                           QVariant &varValue,
                                           QObject *reportItem) {
#if(QT_VERSION < QT_VERSION_CHECK(5, 15, 1))
...
QString ScriptEngineManager::replaceScripts(QString context,
                                            QVariant &varValue,
                                            QObject *reportItem,
                                            ScriptEngineType *se,
...
ScriptEngineManager::ScriptEngineManager() : m_model(0), m_context(0), m_dataManager(0) {
  ...
  QScriptValue fontConstructor = m_scriptEngine->newFunction(QFontPrototype::constructorQFont,
                                                             fontProto);
      ...
      case None:
        if(m_context[curPos] == '$') {
          ...
          if(isStartScriptLexem(curPos))
            extractScript(curPos,
                          substring(m_context, startPos, curPos),
...
void ScriptExtractor::extractScript(int &curPos,
...
void ScriptEngineContext::baseDesignIntfToScript(const QString &pageName, BaseDesignIntf *item) {
  if(item) {
    ...
    QString on = item->patternName().compare(pageName) == 0 ? pageName
...
bool ScriptEngineContext::runInitScript() {
  ...
  if(res.isError()) {
    QMessageBox::critical(0,
                          tr("Error"),
                          QString("Line %1: %2 ")
                              .arg(res.property("lineNumber").toString())
                              .arg(res.toString()));
    return false;
  }
#else
...
QVariant ScriptFunctionsManager::calcGroupFunction(const QString &name,
  ...
  if(m_scriptEngineManager->dataManager()) {
    ...
    GroupFunction *gf = m_scriptEngineManager->dataManager()->groupFunction(name,
                                                                            expression,
                                                                            bandName);
    if(gf) {
      if(gf->isValid()) {
...
QVariant ScriptFunctionsManager::numberFormat(QVariant value,
                                              const char &format,
                                              int precision,
...
QVariant ScriptFunctionsManager::dateFormat(QVariant value,
...
QVariant ScriptFunctionsManager::dateTimeFormat(QVariant value,
                                                const QString &format,
                                                const QString &locale) {
...
QVariant ScriptFunctionsManager::sectotimeFormat(QVariant value, const QString &format) {
  ...
  for(int len = 2; len; len--) {
    if(hasHour)
      result.replace(QString('h').repeated(len), QString::number(hours).rightJustified(len, '0'));
    if(hasMinute)
      result.replace(QString('m').repeated(len),
                     QString::number(hasHour ? minutes % 60 : minutes).rightJustified(len, '0'));
    result.replace(QString('s').repeated(len),
...
QVariant ScriptFunctionsManager::currencyUSBasedFormat(QVariant value,
  ...
  QString CurrencySymbol = (!currencySymbol.isEmpty()) ? currencySymbol
                                                       : QLocale::system().currencySymbol();
  // Format it using USA locale
  QString vTempStr = QLocale(QLocale::English, QLocale::UnitedStates)
                         .toCurrencyString(value.toDouble());
...
void ScriptFunctionsManager::addTableOfContentsItem(const QString &uniqKey,
                                                    const QString &content,
                                                    int indent) {
...
QFont ScriptFunctionsManager::font(
    const QString &family, int pointSize, bool italic, bool bold, bool underLine) {
...
void TableOfContents::setItem(const QString &uniqKey,
                              const QString &content,
                              int pageNumber,
                              int indent) {
  ContentItem *item = 0;
...
void LimeReport::TableOfContents::clear() {
  m_hash.clear();
  foreach(ContentItem *item, m_tableOfContents) { delete item; }
  m_tableOfContents.clear();
}
