...
QueryHolder::QueryHolder(QString queryText, QString connectionName, DataSourceManager *dataManager)
    : m_queryText(queryText), m_connectionName(connectionName), m_mode(IDataSource::RENDER_MODE),
      m_dataManager(dataManager), m_prepared(true) {
  extractParams();
}
...
QVariant ModelToDataSource::dataByRowIndex(const QString &columnName, int rowIndex, int roleName) {
  if(m_model->rowCount() > rowIndex)
    return m_model->data(m_model->index(rowIndex, columnIndexByName(columnName)), roleName);
  return QVariant();
}
...
ConnectionDesc::ConnectionDesc(QSqlDatabase db, QObject *parent)
    : QObject(parent), m_connectionName(db.connectionName()), m_connectionHost(db.hostName()),
      m_connectionDriver(db.driverName()), m_databaseName(db.databaseName()), m_user(db.userName()),
      m_password(db.password()), m_port(""), m_autoconnect(false), m_internal(false),
...
SubQueryHolder::SubQueryHolder(QString queryText,
...
SubQueryDesc::SubQueryDesc(QString queryName,
                           QString queryText,
                           QString connection,
...
ProxyHolder::ProxyHolder(ProxyDesc *desc, DataSourceManager *dataManager)
    : m_model(0), m_desc(desc), m_lastError(""), m_mode(IDataSource::RENDER_MODE), m_invalid(false),
  ...
  if(!m_datasource) {
    if(dataManager()) {
      ...
      if(master && child) {
        ...
        m_lastError.clear();
        if(!master)
          m_lastError += QObject::tr("Master datasouce \"%1\" not found!").arg(m_desc->master());
        if(!child)
...
CSVHolder::CSVHolder(const CSVDesc &desc, DataSourceManager *dataManager)
    : m_csvText(desc.csvText()), m_separator(desc.separator()), m_dataManager(dataManager),
