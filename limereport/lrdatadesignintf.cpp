...
QueryHolder::QueryHolder(QString queryText, QString connectionName, DataSourceManager* dataManager):
    ...
    m_mode(IDataSource::RENDER_MODE),
...
QString QueryHolder::connectionName() { return m_connectionName; }

void QueryHolder::setConnectionName(QString connectionName) { m_connectionName = connectionName; }
...
QString QueryHolder::queryText() { return m_queryText; }
...
void ModelToDataSource::first() { m_curRow = 0; }
...
QVariant ModelToDataSource::dataByKeyField(const QString& columnName, const QString& keyColumnName,
                                           QVariant keyData)
{
...
QString ModelToDataSource::lastError() { return m_lastError; }

QAbstractItemModel* ModelToDataSource::model() { return m_model; }
...
ConnectionDesc::ConnectionDesc(QSqlDatabase db, QObject* parent):
    ...
    m_connectionHost(db.hostName()),
    m_connectionDriver(db.driverName()),
...
void ConnectionDesc::setName(const QString& value)
{
    if (m_connectionName != value)
        emit nameChanged(m_connectionName, value);
    m_connectionName = value;
}

bool ConnectionDesc::isEqual(const QSqlDatabase& db)
{
    return (db.databaseName() == m_databaseName) && (db.driverName() == m_connectionDriver)
        && (db.hostName() == m_connectionHost) && (db.connectionName() == m_connectionName)
        && (db.userName() == m_user) && (db.password() == m_password);
}

QString ConnectionDesc::connectionNameForUser(const QString& connectionName)
{
    return connectionName.compare(QSqlDatabase::defaultConnection) == 0 ? tr("defaultConnection")
                                                                        : connectionName;
}

QString ConnectionDesc::connectionNameForReport(const QString& connectionName)
{
    return connectionName.compare(tr("defaultConnection")) == 0 ? QSqlDatabase::defaultConnection
                                                                : connectionName;
}

QString ConnectionDesc::port() const { return m_port; }

void ConnectionDesc::setPort(QString port) { m_port = port; }

bool ConnectionDesc::keepDBCredentials() const { return m_keepDBCredentials; }
...
SubQueryHolder::SubQueryHolder(QString queryText, QString connectionName, QString masterDatasource,
    ...
    QueryHolder(queryText, connectionName, dataManager),
    m_masterDatasource(masterDatasource) /*, m_invalid(false)*/
{
    extractParams();
}
...
SubQueryDesc::SubQueryDesc(QString queryName, QString queryText, QString connection,
                           QString masterDatasourceName):
    QueryDesc(queryName, queryText, connection),
    m_masterDatasourceName(masterDatasourceName)
{
}
    ...
    if (!m_datasource) {
        ...
        if (dataManager()) {
            ...
            if (master && child) {
                ...
                if (!master)
                    m_lastError
                        += QObject::tr("Master datasouce \"%1\" not found!").arg(m_desc->master());
                if (!child)
                    m_lastError += ((m_lastError.isEmpty()) ? QObject::tr("Child")
                                                            : QObject::tr(" and child "))
...
int MasterDetailProxyModel::fieldIndexByName(QString fieldName) const
{
    for (int i = 0; i < sourceModel()->columnCount(); ++i) {
        QString sourceFieldName
            = sourceModel()->headerData(i, Qt::Horizontal, Qt::UserRole).isValid()
...
QVariant MasterDetailProxyModel::sourceData(QString fieldName, int row) const
{
    ...
    if (fieldIndex != -1) {
        ...
        throw ReportError(tr("Field: \"%1\" not found in \"%2\" child datasource")
                              .arg(fieldName)
                              .arg(m_childName));
    }
}
...
QVariant MasterDetailProxyModel::masterData(QString fieldName) const
{
    ...
    if (master) {
        ...
        if (columnIndex != -1) {
            ...
            throw ReportError(tr("Field: \"%1\" not found in \"%2\" master datasource")
                                  .arg(fieldName)
                                  .arg(m_masterName));
        }
    }
...
QVariant CallbackDatasource::dataByRowIndex(const QString& columnName, int rowIndex)
{
    ...
    if (!eof())
        result = callbackData(columnName, rowIndex);
    first();
...
QVariant CallbackDatasource::dataByKeyField(const QString& columnName, const QString& keyColumnName,
                                            QVariant keyData)
{
    int backupCurrentRow = m_currentRow;
    QVariant result = QVariant();
...
QString CSVDesc::name() const { return m_csvName; }

void CSVDesc::setName(const QString& csvName) { m_csvName = csvName; }

QString CSVDesc::csvText() const { return m_csvText; }
...
void CSVDesc::setSeparator(const QString& separator) { m_separator = separator; }

bool CSVDesc::firstRowIsHeader() const { return m_firstRowIsHeader; }

void CSVDesc::setFirstRowIsHeader(bool firstRowIsHeader) { m_firstRowIsHeader = firstRowIsHeader; }
...
bool CSVHolder::firsRowIsHeader() const { return m_firstRowIsHeader; }

void CSVHolder::setFirsRowIsHeader(bool firstRowIsHeader) { m_firstRowIsHeader = firstRowIsHeader; }
