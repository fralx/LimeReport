...
void DataSourceModel::updateModel() {
  ...
  m_rootNode->clear();
  DataNode *ds = m_rootNode->addChild(tr("Datasources"),
                                      DataNode::DataSources,
                                      QIcon(":/report/images/databases"));

  foreach(QString name, m_dataManager->connectionNames()) {
    DataNode *connection = ds->addChild(name,
                                        DataNode::Connection,
                                        QIcon(":/report/images/database"));
  ...
  foreach(QString name, m_dataManager->dataSourceNames()) {
    ...
    if(m_dataManager->isQuery(name)) {
      ...
      if(connection)
        datasource = connection->addChild(name, DataNode::Query, QIcon(":/report/images/table"));
      else
        datasource = ds->addChild(name, DataNode::Query);
    } else if(m_dataManager->isSubQuery(name)) {
      DataNode *connection = connections.value(
          m_dataManager->subQueryByName(name)->connectionName());
      if(connection)
        datasource = connection->addChild(name, DataNode::SubQuery, QIcon(":/report/images/table"));
      else
  ...
  DataNode *vars = m_rootNode->addChild(tr("Variables"),
                                        DataNode::Variables,
  ...
  vars = m_rootNode->addChild(tr("External variables"),
                              DataNode::Variables,
                              QIcon(":/report/images/folder"));
...
DataSourceManager::DataSourceManager(QObject *parent)
  ...
  m_groupFunctionFactory
      .registerFunctionCreator(QLatin1String("COUNT"),
                               new ConstructorGroupFunctionCreator<CountGroupFunction>);
  m_groupFunctionFactory
      .registerFunctionCreator(QLatin1String("SUM"),
                               new ConstructorGroupFunctionCreator<SumGroupFunction>);
  m_groupFunctionFactory
      .registerFunctionCreator(QLatin1String("AVG"),
                               new ConstructorGroupFunctionCreator<AvgGroupFunction>);
  m_groupFunctionFactory
      .registerFunctionCreator(QLatin1String("MIN"),
                               new ConstructorGroupFunctionCreator<MinGroupFunction>);
  m_groupFunctionFactory
      .registerFunctionCreator(QLatin1String("MAX"),
                               new ConstructorGroupFunctionCreator<MaxGroupFunction>);
  ...
  connect(&m_reportVariables,
          SIGNAL(variableHasBeenAdded(QString)),
          this,
          SLOT(slotVariableHasBeenAdded(QString)));
  connect(&m_reportVariables,
          SIGNAL(variableHasBeenChanged(QString)),
          this,
          SLOT(slotVariableHasBeenChanged(QString)));
  connect(&m_userVariables,
          SIGNAL(variableHasBeenAdded(QString)),
          this,
          SLOT(slotVariableHasBeenAdded(QString)));
  connect(&m_userVariables,
...
QString DataSourceManager::replaceFields(QString query,
                                         QMap<QString, QString> &aliasesToParam,
                                         QString masterDatasource) {
#if QT_VERSION >= QT_VERSION_CHECK(5, 15, 1)
...
void DataSourceManager::addQuery(const QString &name,
                                 const QString &sqlText,
...
void DataSourceManager::addSubQuery(const QString &name,
                                    const QString &sqlText,
                                    const QString &connectionName,
                                    const QString &masterDatasource) {
  SubQueryDesc *subQueryDesc = new SubQueryDesc(name.toLower(),
                                                sqlText,
                                                connectionName,
                                                masterDatasource);
...
void DataSourceManager::addProxy(const QString &name,
...
void DataSourceManager::addCSV(const QString &name,
...
void DataSourceManager::putQueryDesc(QueryDesc *queryDesc) {
  if(!containsDatasource(queryDesc->queryName())) {
    ...
    connect(queryDesc,
            SIGNAL(queryTextChanged(QString, QString)),
            this,
            SLOT(slotQueryTextChanged(QString, QString)));
  } else
    throw ReportError(tr("Datasource with name \"%1\" already exists!").arg(queryDesc->queryName()));
}
...
void DataSourceManager::putCSVDesc(CSVDesc *csvDesc) {
  if(!containsDatasource(csvDesc->name())) {
    ...
    connect(csvDesc,
            SIGNAL(cvsTextChanged(QString, QString)),
            this,
...
bool DataSourceManager::initAndOpenDB(QSqlDatabase &db, ConnectionDesc &connectionDesc) {
  ...
  if(connectionDesc.driver().compare("QSQLITE") == 0) {
    if(!defaultDatabasePath().isEmpty()) {
      dbName = !QFileInfo(dbName).exists() ? defaultDatabasePath() + QFileInfo(dbName).fileName()
                                           : dbName;
    }
    ...
    while(it.hasNext()) {
      ...
      if(!m_datasources.contains(it.value()->queryName().toLower())) {
        connect(it.value(),
                ...
                SLOT(slotQueryTextChanged(QString, QString)));
        putHolder(it.value()->queryName(),
                  new SubQueryHolder(it.value()->queryText(),
...
void DataSourceManager::addVariable(const QString &name,
...
void DataSourceManager::setSystemVariable(const QString &name,
                                          const QVariant &value,
                                          RenderPass pass) {
...
GroupFunction *DataSourceManager::addGroupFunction(const QString &name,
                                                   const QString &expression,
                                                   const QString &band,
                                                   const QString &dataBand) {
...
GroupFunction *DataSourceManager::groupFunction(const QString &name,
                                                const QString &expression,
                                                const QString &band) {
...
void DataSourceManager::updateChildrenData(const QString &datasourceName) {
  foreach(SubQueryDesc *subquery, m_subqueries) {
    if(subquery->master().compare(datasourceName, Qt::CaseInsensitive) == 0) {
      SubQueryHolder *holder = dynamic_cast<SubQueryHolder *>(
          dataSourceHolder(subquery->queryName()));
      if(holder)
        holder->runQuery();
    }
  }
