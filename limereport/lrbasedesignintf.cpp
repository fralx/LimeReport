...
void BaseDesignIntf::paint(QPainter* ppainter, const QStyleOptionGraphicsItem* option,
                           QWidget* widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
void BaseDesignIntf::prepareRect(QPainter* painter, const QStyleOptionGraphicsItem* /*option*/,
                                 QWidget* /*widget*/)
{
    painter->save();
...
void BaseDesignIntf::showEditorDialog() { showDialog(defaultEditor()); }
...
void BaseDesignIntf::contextMenuEvent(QGraphicsSceneContextMenuEvent* event)
{
    ...
    QAction* editBorderAction
        = menu.addAction(QIcon(":/report/images/borderEditor"), tr("Edit borders..."));
    ...
    if (a) {
        ...
        if (a == editBorderAction) {
            ...
            if (be.exec() == QDialog::Rejected)
                return;
            page->setBordersExt(be.borderSides(), be.borderWidth(),
